...
RunSketcher::RunSketcher(const FlatVector &ImageSize, int Downscale) :
	ImageSpace(FlatVector(), ImageSize), PixelsBelow(Downscale), DisplaySpace(FlatVector(), ImageSpace.Size / (float)PixelsBelow),
	Data(new StorageData(ImageSpace.Size))
	{}

RunSketcher::RunSketcher(const String &Filename, const FlatVector &ImageSize, int Downscale) :
	ImageSpace(FlatVector(), ImageSize), PixelsBelow(Downscale), DisplaySpace(FlatVector(), ImageSpace.Size / (float)PixelsBelow),
	Data(NULL)
{
	/// Open the file
	...
	if (Input == NULL)
	{
		std::cerr << Local("Could not open for reading: ") << Filename << std::endl;
		Data = new StorageData(ImageSpace.Size);
		return;
	}
	...
	strncpy(Identifier, "inscribble v00\n", 32);

	char LoadedIdentifier[32];
	fread(LoadedIdentifier, sizeof(char), 32, Input);

	if (strncmp(LoadedIdentifier, Identifier, 32) != 0)
	{
	...
	for (unsigned int CurrentRow = 0; CurrentRow < RowCount; CurrentRow++)
	{
		...
		Data->Rows[CurrentRow].Runs = new StorageData::Run[Data->Rows[CurrentRow].RunCount];
		BZ2_bzRead(&Error, CompressInput, Data->Rows[CurrentRow].Runs, sizeof(StorageData::Run) * Data->Rows[CurrentRow].RunCount);
	}
...
RunSketcher::~RunSketcher(void)
{
	delete Data;
}

bool RunSketcher::Save(const String &Filename)
{
	/// Open the file
	...
	for (unsigned int CurrentRow = 0; CurrentRow < Data->RowCount; CurrentRow++)
	{
		...
		unsigned int RunBufferSize = sizeof(StorageData::Run) * Data->Rows[CurrentRow].RunCount;
...
bool RunSketcher::Export(const String &Filename, const Color &Foreground, const Color &Background, int Scale)
{
	...
	if (!RenderInternal(Region(FlatVector(), ExportSize), ExportContext, Scale, Foreground, Background))
	{
...
Region RunSketcher::Mark(const CursorState &Start, const CursorState &End, const bool &Black)
{
	...
	struct LineProperties
	{
		...
		LineProperties(const FlatVector &Start, const FlatVector &End, const bool &IsRightHandLine) :
	...
	struct Cap
	{
		...
		Cap(const FlatVector &Center, float const &Radius) :
...
bool RunSketcher::Render(const Region &Invalid, cairo_t *Destination, const Color &Foreground, const Color &Background)
{
	return RenderInternal(Invalid, Destination, PixelsBelow, Foreground, Background);
}

int RunSketcher::Zoom(int Amount)
{
...
FlatVector &RunSketcher::GetDisplaySize(void)
	{ return DisplaySpace.Size; }

bool RunSketcher::HasChanges(void) { return false; }
void RunSketcher::Undo(void) {}
void RunSketcher::Redo(void) {}

bool RunSketcher::RenderInternal(const Region &Invalid, cairo_t *Destination, int Scale,
	const Color &Foreground, const Color &Background)
{
	/// Prepare a buffer before we copy to the screen
	const Region CanvasInvalid = DisplaySpace.Intersect(Region(Invalid.Start, Invalid.Size + FlatVector(1, 1)));
	const int InvalidX = (int)CanvasInvalid.Start[0],
		InvalidY = (int)CanvasInvalid.Start[1],
		InvalidWidth = (int)CanvasInvalid.Size[0],
		InvalidHeight = (int)CanvasInvalid.Size[1],
		InvalidColumnEnd = InvalidX + InvalidWidth,
		InvalidRowEnd = InvalidY + InvalidHeight;

	if ((InvalidWidth <= 0) || (InvalidHeight <= 0)) return true;

	int Stride = cairo_format_stride_for_width(CAIRO_FORMAT_ARGB32, InvalidWidth);
	void *Pixels = new unsigned char[Stride * InvalidHeight];
	...
	uint32_t Colors[ShadeCount]; // Shades between background and foreground colors. 0 is bg, Scale * Scale is fg.
	for (unsigned int CurrentColor = 0; CurrentColor < ShadeCount; CurrentColor++)
	{
		...
		const Color Intermediary(Background, Foreground, (float)CurrentColor * ShadeUnitScale);
		Colors[CurrentColor] =
	...
	unsigned int LineShades[InvalidWidth];

	void *CurrentPixelByte = Pixels;
	for (unsigned int CurrentRow = 0; CurrentRow < InvalidHeight; CurrentRow++)
	{
		// Blank the row
		for (unsigned int CurrentColumn = 0; CurrentColumn < InvalidWidth; CurrentColumn++)
			{ LineShades[CurrentColumn] = 0; }
	...
	cairo_surface_t *CopySurface = cairo_image_surface_create_for_data(
		(unsigned char *)Pixels, CAIRO_FORMAT_ARGB32, InvalidWidth, InvalidHeight, Stride);
...
///////////////////////////////////////
// RLE data management and modification
RunSketcher::StorageData::StorageData(const FlatVector &Size) :
...
RunSketcher::StorageData::~StorageData(void)
{
	for (int CurrentRow = 0; CurrentRow < RowCount; CurrentRow++)
...
void RunSketcher::StorageData::Line(int UnclippedLeft, int UnclippedRight, int const Y, bool Black)
{
	// Validate parameters
	if (Y < 0) return;
	...
	Run *&Runs = Rows[Y].Runs = new Run[RunCount + 1];
#ifndef NDEBUG
	unsigned int const AllocatedRunCount = RunCount + 1; // DEBUG ONLY
#endif
	RunCount = 0;
	...
	else
	{
		...
		for (int OldRun = 0; OldRun < OldRunCount; OldRun++)
		{
...
void RunSketcher::StorageData::Combine(unsigned int *Buffer, unsigned int const BufferWidth,
	unsigned int const X, unsigned int const Y, unsigned int const Scale)
{
	...
	{
		StorageData::Row const& CurrentRow = Rows[CurrentRowIndex];
		...
		for (unsigned int CurrentRunIndex = 0; CurrentRunIndex < CurrentRow.RunCount; CurrentRunIndex++, CurrentRun++)
		{
			...
			else
			{
				...
				for (int BufferPixel = ScreenStart; BufferPixel < ScreenStop; BufferPixel++)
				{
